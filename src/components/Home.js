...
const Home = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    const response = await nlightnApi.pythonApp("staffing_optimizer", "process_staff_data", formData);
  ...
  return (
    <div className={`flex flex-col w-full h-[100%] items-center body-mode-${mode} overflow-y-auto`}>
      
      <h1 className={`flex w-[90%] text-[32px]`}>Event Staffing Optimizer</h1>
      ...
      {staffData.length>0 &&
        <div className={`flex flex-col w-[75%] fade-in`}>
          ...
          <div className={`flex w-full h-[100%] shadow-md rounded-md border-mode-${mode} rounded-md shadow-mode-${mode} transition duration-500`}>
